...
def login():
    """
    Handles user login and cookie creation.
...
def new_chat():
    ...
    if recipient_user.id not in [
        user_chat["user_id"] for user_chat in existing_chat.chat_list
    ]:
...
def chat():
    ...
    messages = (
        Message.query.filter_by(room_id=room_id).first().messages if room_id else []
    )
...
def ftime(date):
    # Ensure that date is a float or string that can be converted to a float
    timestamp = float(
        date
    )  # Convert to float in case the date is a string with decimal
    dt = datetime.fromtimestamp(int(timestamp))  # Convert to integer timestamp
...
def visualize():
    ...
    Returns:
        Response: Flask response object.
    """
    from io import BytesIO  # Import BytesIO to handle image buffer
    ...
    series = pd.Series(registered_users)
    ...
    # Render the template
    return render_template("visualize.html", chart=plot_url)
